...
np = {
	...
	zeros, eye, arange, vstack, hstack, NDArray, linalg, linspace, random, cumsum, mean
}
...
// const query = "SELECT `TimeStamp`, `value` FROM `APPLICATION_74`  order by `TimeStamp`  limit 100";
const query = "SELECT `TimeStamp`, `id`, `carID`, `carPlates`, `ownerName` FROM `APPLICATION_74`  order by `id`  limit 100";

SearchInAsync(query).then((res) => {
    // event.log(JSON.stringify(res));
    mod = new AutoRegressionIntegratedMovingAverage(1, 1, 1);
    return mod.fit(res, 256);
}).then(res => {
  // return res.predict(2);
// }).then(pred => {
  // console.log(pred);
  // event.end();
    DataListAddAsync("iti2021", res);
    event.end();
}).catch((err) => {
    event.error(err);
});
